...
**MAS Testing Profiles:** Seit September 2026 nicht mehr Teil des MASTG, sondern eigener Bereich der MAS-Website (Repo `OWASP/mas-website`, Ordner `docs/Profiles`). Prüfe auf neue oder umbenannte Profile und darauf, ob ein Profil als Standard- oder als Spezialprofil geführt wird (Spezialprofile stehen dort unter „Specialized Profiles“; derzeit einziges Spezialprofil ist MAS-EUDIW, veröffentlicht im September 2026):
...
**Profilzuordnung der Tests:** Die aktuellen MASTG-Tests tragen seit September 2026 keine `profiles`-Metadaten mehr, sie erben sie von der zugehörigen MASWE (deren Frontmatter das Feld `profiles` führt). Wer prüfen will, welche Tests ein Profil verlangt, liest die Profile an der Weakness. Ein `profiles`-Feld in einem Test ist ein Rest der als `deprecated` markierten MASTG-V1-Generation und keine Gegenanzeige. Prüfe bei jedem Sync, ob diese Regel noch gilt, bevor Test-Mappings aktualisiert werden.

### Rebuild: references/mastg-testing.md

Betroffene Sektionen aktualisieren:
- Testing-Profile: Profiltabelle (Ziel, Angreifermodell), Kombinationen und die Tabelle der Spezialprofile mit mas.owasp.org/Profiles abgleichen
